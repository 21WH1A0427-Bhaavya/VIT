...
BEGIN
    ...
    IF disease_count > 1 THEN
        ...
        VALUES ('Genetic Correlation',
                CONCAT('Gene ID ', NEW.gene_id, ' is associated with ', disease_count, ' diseases — multi-pathogenic correlation detected.'),
                CURDATE());
    END IF;
END$$
...
-- 1. screening_date_check: Expect ERROR (date in future) -> This should raise an error and stop the insert
INSERT INTO Screening (patient_id, mutation_id, test_date, result)
VALUES (1, 1, DATE_ADD(CURDATE(), INTERVAL 5 DAY), 'Positive');

-- 2. screening_date_update: updating an existing screening to a future date -> Expect ERROR
UPDATE Screening
SET test_date = DATE_ADD(CURDATE(), INTERVAL 2 DAY)
WHERE screening_id = 1;

-- 3. trg_family_risk (family propagation): Add new positive screening for a patient with family in DB
INSERT INTO Screening (patient_id, mutation_id, test_date, result)
...
-- 5. trg_positive_test_outreach: Positive test in a region (auto-create awareness program if none exists today)
INSERT INTO Screening (patient_id, mutation_id, test_date, result)
VALUES (10, 3, CURDATE(), 'Positive');
SELECT * FROM Awareness_Programs ORDER BY program_id DESC LIMIT 1;

-- 6. trg_screening_update: Update existing screening from Negative → Positive then back to Negative
...
-- 8. trg_mutation_hotspot_detection: add multiple positives for same mutation in same region to produce hotspot
INSERT INTO Screening (patient_id, mutation_id, test_date, result)
VALUES
...
-- 10. trg_family_risk_alert: demo family correlation without hardcoding IDs
...
-- insert screening for Rohan (positive baseline) using SELECT to find assigned id
INSERT INTO Screening (patient_id, mutation_id, test_date, result)
VALUES (
    (SELECT patient_id FROM Patients WHERE name = 'Rohan Kumar' LIMIT 1),
    1, CURDATE(), 'Positive'
);
-- insert screening for Meera (should trigger family correlation alert)
INSERT INTO Screening (patient_id, mutation_id, test_date, result)
VALUES (
